...
class MainProcess:
    def __init__(self, root):               
        ...
        sizeWw = str(ii[3])      
        imgUSER = PhotoImage(file=f"cross{sizeWw}.png")
    ...
    def waitSaveTheFlag(self):
        ...
        if self.editIt=='True':
            ...
            newLine = f'{seqName}|||{cent}|||{oldFlag}|||{wW}|||{hH}|||{wait}|||{gan}|||{tol}|||TAB10'            
            ...
            proName = self.projectName   
            newLine = f'{seqName}|||{cent}|||./dataProjects/{self.projectName}/flag_{countId}.png|||{wW}|||{hH}|||{wait}|||{gan}|||{tol}|||TAB10\n'
            controlID = editLine
            ...
            f.write(f's{curId}:#$%^&*(st)|||{seqName}|||{cent}|||./dataProjects/{self.projectName}/flag_{countId}.png|||{wW}|||{hH}|||{wait}|||{gan}|||{tol}|||TAB10\n')
            f.close()
            os.system(f'start python workspace.py -p {self.projectName}')        
            sys.exit()
